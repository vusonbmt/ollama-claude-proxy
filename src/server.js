...
app.post("/v1/messages", async (req, res) => {
  try {
    ...
    if (apiKeys.length === 0) {
      return res.status(401).json({
        ...
        error: {
          type: "authentication_error",
          message:
            "API key required. Set OLLAMA_API_KEY environment variable or apiKeys in config.json.",
        },
      });
    }
    ...
    if (modelMapping[requestedModel]) {
      ...
      const mapped =
        typeof targetModel === "string" ? targetModel : targetModel.mapping;
...
// OpenAI-compatible /v1/chat/completions endpoint (used by OpenCode custom providers)
app.post("/v1/chat/completions", async (req, res) => {
  try {
    ...
    const { model, messages, stream, temperature, top_p, max_tokens } =
      req.body;
    ...
    // Resolve model with mapping support
    let requestedModel = model || config.get("defaultModel") || DEFAULT_MODEL;
    const modelMapping = config.get("modelMapping") || {};
    if (modelMapping[requestedModel]) {
      const targetModel = modelMapping[requestedModel];
    ...
    logger.info(
      `[API] OpenAI-compat request for model: ${requestedModel}, stream: ${!!stream}`,
    );
    ...
    if (stream) {
      ...
      try {
        const generator = sendOpenAIMessageStream(request);
        ...
        if (!res.headersSent) {
          return res
            .status(500)
            .json({ error: { type: errorType, message: errorMessage } });
        }
        res.write(
          `data: ${JSON.stringify({ error: { type: errorType, message: errorMessage } })}\n\n`,
        );
        res.end();
      }
    } else {
      const response = await sendOpenAIMessage(request);
      res.json(response);
    }
  } catch (error) {
